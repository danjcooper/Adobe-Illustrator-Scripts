...
function setDate() {
    ...
    monthIndex = monthIndex + 1 // it returns the month as january being 0, so this just changes it so jan = 1
        ...
        if(monthIndex < 10) { // if it's less then 10 make it display as 01 instead of 1
            monthIndex = '0' + monthIndex
        } 
        
    // update the document with the relevant info.
    app.activeDocument.textFrames.getByName ("Ord_DATE").contents = day + '/' + monthIndex + '/' + year;
    ...
    // Number
    var newNumber = newName.replace(newName, "");
    
    // Update Proof to new Varables
    app.activeDocument.textFrames.getByName ("ORDER_NAME").contents = newName;
